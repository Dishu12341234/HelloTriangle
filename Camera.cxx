                       ...
                       Event &event)
{
    ...
    if (event.getKeyPressed(GLFW_KEY_W))          cameraPos += forwardFlat * speed;
    if (event.getKeyPressed(GLFW_KEY_S))          cameraPos -= forwardFlat * speed;
    if (event.getKeyPressed(GLFW_KEY_A))          cameraPos -= right       * speed;
    if (event.getKeyPressed(GLFW_KEY_D))          cameraPos += right       * speed;
    if (event.getKeyPressed(GLFW_KEY_SPACE))      cameraPos.z += speed;
    if (event.getKeyPressed(GLFW_KEY_LEFT_SHIFT)) cameraPos.z -= speed;
                      ...
                      uint32_t &currentFrame)
{
    ...
    // ---------------------------------------------------------------
    glm::vec3 rayOriginVox = rayOriginCam * 10.f;

    const float maxDistVox = 20.0f; // max reach in voxel units
    ...
    if (rayDir.x < 0) { step.x = -1; sideDist.x = (rayOriginVox.x - voxel.x)        * deltaDist.x; }
    else              { step.x =  1; sideDist.x = (voxel.x + 1.0f - rayOriginVox.x) * deltaDist.x; }

    if (rayDir.y < 0) { step.y = -1; sideDist.y = (rayOriginVox.y - voxel.y)        * deltaDist.y; }
    else              { step.y =  1; sideDist.y = (voxel.y + 1.0f - rayOriginVox.y) * deltaDist.y; }

    if (rayDir.z < 0) { step.z = -1; sideDist.z = (rayOriginVox.z - voxel.z)        * deltaDist.z; }
    else              { step.z =  1; sideDist.z = (voxel.z + 1.0f - rayOriginVox.z) * deltaDist.z; }
    ...
    glm::mat4 model =
        glm::translate(glm::mat4(1.0f), rayOriginCam) *
        rotation *
